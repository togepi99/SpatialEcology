...
Looking at the summary stats, it looks like all of the models are terrible as none of them has a kappa or tss that exceeds 1.4. The `glm` and `maxent` models have all together the best proportions of sensitivity, specificity, tss, and kappa. However, the `maxent` calibration plot does not follow the 1:1 slope very well, similar to `glm`. 
...
```{r}
modelLayers <- c(glmMap, gamMap, boostMap, rfMap)
names(modelLayers) <- c('glmMap', 'gamMap', 'boostMap', 'rfMap')

aucVec <- NULL

for(i in 1:4){
  auc = auc(valData, which.model = i+1)
  aucVec <- c(aucVec, auc$AUC)
}

weightedLayers <- weighted.mean(modelLayers, aucVec)

plot(weightedLayers)
```

`bioclim` is the worst model. I don't know why `maxent` is left out because it seems relatively decent.
