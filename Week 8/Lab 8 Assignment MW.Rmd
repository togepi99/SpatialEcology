...
$\color{red}{\text{What are those different ways? +3}}$
...
$\color{red}{\text{Why does it seem more useful? +3.5}}$
...
$\color{red}{\text{The PA model is certainly giving larger predicted values, but this doesn't at all mean that it has stronger predictive power. The major difference here is in the number of background / absence points. The predicted values for the presence-background points are a function of how many background points you choose, and that's why the large discrepancy exists. Otherwise, I'd argue that the patterns of predicted responses as a function of the covariates are relatively similar between the two models. +3}}$
